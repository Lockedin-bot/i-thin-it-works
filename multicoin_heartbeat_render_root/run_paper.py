...
def main():
    ...
    if not symbols:
        log.error("No symbols configured. Set SYMBOLS=... in your .env"); return
    ...
    gran = tf_map.get(s.TA_TIMEFRAME, 300)
            ...
            for sym in symbols:
                ...
                if df is None or len(df) < max(s.FAST_MA, s.SLOW_MA) + 2:
                    ...
                    with open(hb_path, "a", encoding="utf-8") as hb:
                        hb.write(f"{datetime.now(timezone.utc).isoformat()} {msg}\n")
                    continue

                df = momentum_signals(df, fast=s.FAST_MA, slow=s.SLOW_MA)
                last = df.iloc[-1]

                price = float(last["close"])
                fast = float(last["fast"]) if pd.notna(last["fast"]) else float("nan")
                slow = float(last["slow"]) if pd.notna(last["slow"]) else float("nan")
                signal = int(last["signal"]) if pd.notna(last["signal"]) else 0

                msg = f"HEARTBEAT {sym} price={price:.4f} fast={fast:.4f} slow={slow:.4f} signal={signal} pos={position[sym]} equity={equity[sym]:.2f}"
                log.info(msg); print(msg, flush=True)
                with open(hb_path, "a", encoding="utf-8") as hb:
                    hb.write(f"{datetime.now(timezone.utc).isoformat()} {msg}\n")

                stop = slow if pd.notna(slow) else price * 0.9

                if signal == 1 and position[sym] <= 0:
                    units = position_size(equity[sym], s.RISK_PER_TRADE, entry=price, stop=stop)
                    cost = units * price
                    if units > 0 and cost <= equity[sym]:
                        equity[sym] -= cost
                        position[sym] += units
                        ...
                        skip_msg = f"SKIP {sym} BUY units={units} cost={cost:.2f} equity={equity[sym]:.2f}"
                        log.info(skip_msg); print(skip_msg, flush=True)

                elif signal == -1 and position[sym] > 0:
                    proceeds = position[sym] * price
                    equity[sym] += proceeds
